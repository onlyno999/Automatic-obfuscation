...
const UUID = '1f9d104e-ca0e-4202-ba4b-a0afb969c748';
// 反向代理IP，无法访问时通过代理访问
const DEFAULT_PROXY_IP = 'wok.woxxxxxx.nyc.mn:443'; // 来源：https://ipdb.030101.xyz/bestdomain/
// 优选域名/IP
const BEST_DOMAINS = ['bestcf.030101.xyz:443', 'japan.com:443', 'www.visa.com.sg:443', "www.visa.com.hk:443", "icook.hk:443", "icook.tw:443"]

// 要伪装成的网页（根路径访问时会去抓它的 HTML 并返回）：
const MASQUERADE_URL = 'https://cf-worker-dir-bke.pages.dev/';

export default {
    async fetch(req) {
        const u = new URL(req.url);

        // 如果是 websocket 升级请求，走代理逻辑
        if (req.headers.get('Upgrade')?.toLowerCase() === 'websocket') {
            return await handle_ws(req);
        } else if (req.method === 'GET') {
            // 根路径：返回伪装页面（从 MASQUERADE_URL 拉取并注入 base）
            if (u.pathname === '/') {
                return await serve_masquerade(req);
...
async function serve_masquerade(req) {
    try {
        // 抓取伪装页
        const resp = await fetch(MASQUERADE_URL, {
            headers: {
                // 可按需传递客户端UA等
                'User-Agent': req.headers.get('User-Agent') || 'cf-worker-masquerade'
            }
        });

        const contentType = resp.headers.get('content-type') || '';

        // 如果是 HTML，就注入 <base> 让相对路径相对于伪装站点解析
        if (contentType.includes('text/html')) {
            let html = await resp.text();

            // 插入 <base href="..."> 到 <head> 的起始位置（如果没有 head，就放到 html 开头）
            const baseTag = `<base href="${MASQUERADE_URL}">`;
            if (/<head[^>]*>/i.test(html)) {
...
            // 可选：额外注入一个小脚本，防止原站把页面重定向或弹窗（视需要可删）
            // html = html.replace('</head>', '<script>/* masquerade guard */</script></head>');

            // 返回修改后的 HTML，保留主要 header（但删除 content-length 等）
...
        } else {
            // 非 HTML（图片或其他），直接把原始响应转发给客户端
            // 注意：某些跨域/权限 header 可能被 Cloudflare Worker 自动调整
            const headers = new Headers(resp.headers);
            headers.set('Cache-Control', 'no-store, no-cache, must-revalidate, max-age=0');
...
    } catch (err) {
        // 兜底错误展示
        return new Response('<h1>伪装页面加载失败</h1>', {
            status: 502,
...
    let links = [];
    let i = 0;
    const wsPath = '/?ed=2560';   // 已修改，不再 encode，直接输出 /?ed=2560
